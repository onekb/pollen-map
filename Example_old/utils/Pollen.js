//计算坐标
import { copyArrayObject } from "./utils"
import { GatherCircles } from "./src/GatherCircles"
...
class Pollen {
  ...
  _point = []//原始数据
  _pointZombie = []//用于计算 排序啥的 会更改位置 就叫它僵尸数组吧
  ...
  setPoint(point, onlyZombie = false) {
    if (!onlyZombie)
      this._point = copyArrayObject(point)
    this._pointZombie = copyArrayObject(point)
  }
